...
function cpajax_add_javascripts() {
	...
	$vars['cpajax_spinner_url'] = plugins_url( 'includes/commentpress-ajax/assets/images/loading.gif', COMMENTPRESS_PLUGIN_FILE );
	...
	if ( $commentpress_core->pages_legacy->is_special_page() ) {
		...
		wp_enqueue_script(
			'cpajax',
			plugins_url( 'includes/commentpress-ajax/assets/js/cp-ajax-comments-page' . $debug_state . '.js', COMMENTPRESS_PLUGIN_FILE ),
		...
		wp_enqueue_script(
			'cpajax',
			plugins_url( 'includes/commentpress-ajax/assets/js/cp-ajax-comments' . $debug_state . '.js', COMMENTPRESS_PLUGIN_FILE ),
...
function cpajax_plugin_can_activate() {
	...
	if ( ! $commentpress_core->is_commentable() ) {
		$allowed = false;
	}

	// But, allow General Comments Page.
	if ( $commentpress_core->db->option_get( 'cp_general_comments_page' ) == $post->ID ) {
		$allowed = true;
	}
...
function cpajax_reassign_comment() {
	...
	if ( ! empty( $text_sig ) && ! empty( $comment_id ) ) {
		...
		cpajax_reassign_comment_children( (int) $comment_id, $text_sig, $comment_ids );
...
function cpajax_infinite_scroll_scripts() {
	...
	wp_enqueue_script(
		'cpajax-waypoints',
		plugins_url( 'includes/commentpress-ajax/assets/js/jquery.waypoints' . $debug_state . '.js', COMMENTPRESS_PLUGIN_FILE ),
		[ 'jquery' ], // Dependencies.
		COMMENTPRESS_VERSION, // Version.
		true
	);
	...
	wp_enqueue_script(
		'cpajax-infinite',
		plugins_url( 'includes/commentpress-ajax/assets/js/cp-ajax-infinite' . $debug_state . '.js', COMMENTPRESS_PLUGIN_FILE ),
